...
export default function DashboardPage() {
  const { user, ready, cancelMembership, refresh } = useAuth();
  ...
  // Pull latest orders (e.g. straight after a Stripe payment)
  useEffect(() => {
          ...
          {m ? (
            <>
              ...
              <p className="mt-4 rounded-md bg-gold/10 px-3 py-2 text-xs text-navy">
                Your 20% conference discount is applied automatically at checkout.
              </p>
              ...
              {cancelError && (
                <p role="alert" className="mt-3 rounded-md bg-red-50 px-3 py-2 text-xs font-medium text-red-700">{cancelError}</p>
              )}
            </>
          ) : (
            <>
              <p className="mt-4 text-sm text-slate-600">
                You don&apos;t have an active membership. Join to unlock a 20%
                discount on every conference ticket and 10% off courses.
              </p>
