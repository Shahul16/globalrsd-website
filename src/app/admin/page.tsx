...
const SCHEMAS: Record<string, { label: string; idKey: string; fields: Field[]; template: Item; advanced?: string[] }> = {
  events: {
    ...
    fields: [
      ...
      { key: "category", label: "Category", type: "select", options: ["Research Conferences", "Skills Development Workshops"] },
      ...
      { key: "image", label: "Cover image path", type: "text", hint: "e.g. /ev-icmdr-2026.png" },
  ...
  courses: {
    ...
    fields: [
      ...
      { key: "image", label: "Cover image path", type: "text", hint: "e.g. /co-ai-in-cybersecurity.png" },
      { key: "summary", label: "Card summary", type: "textarea", full: true },
      { key: "description", label: "Full description", type: "textarea", full: true },
    ...
    template: {
      ...
      price: 295, category: "Skills Development Courses", image: "/co-generic.png", summary: "", description: "",
...
export default function AdminPage() {
  ...
  const [conn, setConn] = useState<{ ok: boolean; message: string; repo?: string | null; branch?: string; env?: Record<string, boolean> } | null>(null);
  ...
  async function checkConnection() {
    setCheckingConn(true); setConn(null);
    try {
      const res = await fetch(`/api/admin/status?key=${encodeURIComponent(key)}`);
      const d = await res.json().catch(() => ({}));
      setConn({ ok: !!d.ok, message: d.message ?? d.error ?? "Unknown response", repo: d.repo, branch: d.branch, env: d.env });
    } catch {
      setConn({ ok: false, message: "Network error — try again." });
    }
    ...
    <section className="mx-auto max-w-5xl px-4 py-12">
      ...
      <div className="mt-5 rounded-md border border-line p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-navy">Publishing connection</p>
            <p className="text-xs text-muted">Check whether this admin panel can actually push changes to the live site right now.</p>
          </div>
          <button onClick={checkConnection} disabled={checkingConn} className="btn-outline px-3 py-1.5 text-sm disabled:opacity-60">
            {checkingConn ? "Checking…" : "Check connection"}
          </button>
        </div>
        {conn && (
          <div className={`mt-3 rounded-md px-4 py-3 text-sm ${conn.ok ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-700"}`}>
            <p className="font-medium">{conn.message}</p>
            {conn.env && (
              <p className="mt-2 text-xs opacity-80">
                ADMIN_KEY {conn.env.ADMIN_KEY ? "✓" : "✗"} · GITHUB_TOKEN {conn.env.GITHUB_TOKEN ? "✓" : "✗"} · GITHUB_REPO {conn.env.GITHUB_REPO ? "✓" : "✗"}
                {conn.repo ? ` (${conn.repo}@${conn.branch})` : ""}
              </p>
            )}
          </div>
        )}
