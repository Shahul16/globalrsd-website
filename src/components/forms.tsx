...
/**
 * Gets a reCAPTCHA v3 token for this submission, if the site key is
 * configured and the script has loaded (see RecaptchaLoader). Resolves to ""
 * if reCAPTCHA isn't set up — the server treats a missing token as "not
 * configured" and lets the submission through unprotected, same as every
 ...
 * /api/lead, which emails them to the admin inbox via Resend. Every
 * submission is first tagged with a reCAPTCHA v3 token (silently skipped if
 * reCAPTCHA isn't configured) so /api/lead can screen out bots.
 */
export function DemoForm({
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const action = name.toLowerCase().replace(/[^a-z0-9]+/g, "_").slice(0, 40) || "form_submit";
    const token = await getRecaptchaToken(action);
    if (token) {
      data.set("recaptchaToken", token);
      data.set("recaptchaAction", action);
    }
