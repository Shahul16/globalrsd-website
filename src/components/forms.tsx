 ...
 * submission is tagged with a Cloudflare Turnstile token and/or reCAPTCHA token
 * (silently skipped if not configured) so /api/lead can screen out bots.
 */
export function DemoForm({
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    // If Turnstile token was generated by widget
    const formTurnstile = String(data.get("cf-turnstile-response") || turnstileToken || "");
    if (formTurnstile) {
      data.set("turnstileToken", formTurnstile);
      data.set("cf-turnstile-response", formTurnstile);
    }
  ...
  return (
    <form onSubmit={onSubmit} aria-label={name} className="space-y-5">
      ...
      {/* Cloudflare Turnstile bot challenge (silently skipped if NEXT_PUBLIC_TURNSTILE_SITE_KEY is not set) */}
      <Turnstile
        action={name.toLowerCase().replace(/[^a-z0-9]+/g, "_").slice(0, 40)}
